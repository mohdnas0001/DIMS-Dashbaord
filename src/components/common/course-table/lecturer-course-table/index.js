...
const LecturerCourseTable = ({ courses }) => {
  return (
    <div className="p-6 my-8 bg-white border rounded-lg">
     <div className="flex items-center gap-4 my-4">
        <h1 className='text-lg font-medium text-black'>Class Schedule(Weekly)</h1>
          ...
          {courses.map((course, index) => (
            <tr key={index} className="text-sm">
              ...
              <td className="p-2 border-b">
                <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold ${course.status === 'Available' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                  <span className={`inline-block w-2 h-2 mr-1 rounded-full ${course.status === 'Available' ? 'bg-green-500' : 'bg-red-500'}`} />
                  {course.status}
                </span>
              </td>
